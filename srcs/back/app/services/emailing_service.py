...
class EmailingService :
  def resend_email(email: str, username : str, token: str) :
      msg = Message(
          subject="Confirmation resend",
          recipients=[email],
      )
      link = Config.VITE_BACKEND_LINK + "/user/verify_account?token_id=" + token 
      msg.body = f"""
      Hello { username },
      Welcome to Matcha! To activate your account, visit the link below:
      { link }
      If you didn't requrst this link, please ignore this email. Contact us if you need any help!
      """
